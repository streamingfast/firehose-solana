...
var StartCmd = &cobra.Command{Use: "start", Short: "Starts `dfuse for Solana` services all at once", RunE: dfuseStartE, Args: cobra.ArbitraryArgs}
...
func dfuseStartE(cmd *cobra.Command, args []string) (err error) {
	...
	userLog.Debug("dfusesol binary started", zap.String("data_dir", dataDir))

	configFile := viper.GetString("global-config-file")
	userLog.Printf("Starting dfuse for Solana with config file '%s'", configFile)
