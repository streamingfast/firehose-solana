...
func init() {
	...
	launcher.RegisterApp(zlog, &launcher.AppDef{
		...
		RegisterFlags: func(cmd *cobra.Command) error {
			...
			cmd.Flags().String("reader-node-data-dir", fmt.Sprintf("{data-dir}/%s/data", "reader"), "Directory for data (node blocks and state)")
			...
			cmd.Flags().String("reader-node-block-data-working-dir", "{data-dir}/reader/block-data-work", "Path where reader will stores its files")
			...
			cmd.Flags().String("reader-node-one-block-suffix", "", "If non-empty, the oneblock files will be appended with that suffix, so that readers can each write their file for a given block instead of competing for writes.")
